...
# 3. 选择降水变量（原单位：米）
precipitation_sum = dataset.select(
...
print(f"已设置下载 {start_year} 至 {end_year} 年的月度降水数据，共 {len(year_months)} 个月")
...
for i, (year, month) in enumerate(year_months):
    ...
    task = ee.batch.Export.image.toDrive(
        image=image,
        description=f"total_precipitation_sum_{year}{month_str}",
        fileNamePrefix=f"total_precipitation_sum_{year}{month_str}",
        folder='total_precipitation_sum_monthly',
        region=roi.geometry(),
        scale=1000,  # 分辨率（米）
        crs='EPSG:4326',
        maxPixels=1e13
    ...
    task.start()
    print(
        f"已提交导出任务：total_precipitation_sum_{year}{month_str}")

...
# 3. 选择温度变量
temperature2m = dataset.select('temperature_2m')
...
print(f"已设置下载 {start_year} 至 {end_year} 年的月度温度数据，共 {len(year_months)} 个月")
...
for i, (year, month) in enumerate(year_months):
    ...
    task = ee.batch.Export.image.toDrive(
        image=image,
        description=f"temperature_2m_{year}{month_str}",
        fileNamePrefix=f"temperature_2m_{year}{month_str}",
        folder='temperature_2m_monthly',
        region=roi.geometry(),
        scale=1000,  # 分辨率（米）
        crs='EPSG:4326',
        maxPixels=1e13
    ...
    task.start()
    print(
        f"已提交导出任务：temperature_2m_{year}{month_str}")

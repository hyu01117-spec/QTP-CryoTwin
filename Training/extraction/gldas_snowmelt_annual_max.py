...
output_file_1980_2003 = os.path.join(output_dir, "GLDAS_snowmelt_max_1980-2003.tif")
output_file_2004_2024 = os.path.join(output_dir, "GLDAS_snowmelt_max_2004-2024.tif")
...
# --- 6. 根据年份进行分组和计算最大值 ---
# 找到两个时间段的年份
years_1980_2003 = [year for year in all_years if 1980 <= year <= 2003]
years_2004_2024 = [year for year in all_years if 2004 <= year <= 2024]

max_data_1980_2003 = None
max_data_2004_2024 = None

# 计算1980-2003年最大值
print("开始计算 1980-2003 年的最大值...")
if years_1980_2003:
    # 检查所有年份的数据尺寸是否一致
    first_year = years_1980_2003[0]
    target_shape = yearly_max[first_year]['data'].shape
    all_same_shape = True
    
    for year in years_1980_2003:
    ...
    if all_same_shape:
        # 堆叠所有年份数据
        period_data = [yearly_max[year]['data'] for year in years_1980_2003]
        period_stack = np.stack(period_data, axis=0)
        max_data_1980_2003 = np.nanmax(period_stack, axis=0)
        print(f"已计算 1980-2003 年最大值 ({max_data_1980_2003.shape[0]}×{max_data_1980_2003.shape[1]})")
    else:
        print("警告：1980-2003时间段内数据尺寸不一致，无法计算多年最大值。")
else:
    print("警告：1980-2003时间段没有数据。")

# 计算2004-2024年最大值
print("开始计算 2004-2024 年的最大值...")
if years_2004_2024:
    # 检查所有年份的数据尺寸是否一致
    first_year = years_2004_2024[0]
    target_shape = yearly_max[first_year]['data'].shape
    all_same_shape = True
    
    for year in years_2004_2024:
        if yearly_max[year]['data'].shape != target_shape:
            print(f"警告：年份 {year} 的数据尺寸 {yearly_max[year]['data'].shape} 与其他年份 {target_shape} 不一致，将跳过该年份。")
            all_same_shape = False
            break
    ...
    if all_same_shape:
        # 堆叠所有年份数据
        period_data = [yearly_max[year]['data'] for year in years_2004_2024]
        period_stack = np.stack(period_data, axis=0)
        max_data_2004_2024 = np.nanmax(period_stack, axis=0)
        print(f"已计算 2004-2024 年最大值 ({max_data_2004_2024.shape[0]}×{max_data_2004_2024.shape[1]})")
    else:
        print("警告：2004-2024时间段内数据尺寸不一致，无法计算多年最大值。")
else:
    print("警告：2004-2024时间段没有数据。")
...
if max_data_1980_2003 is not None and years_1980_2003:
    # 使用第一个年份的元数据
    first_year = years_1980_2003[0]
    save_tiff(output_file_1980_2003, max_data_1980_2003, yearly_max[first_year]['meta'])

if max_data_2004_2024 is not None and years_2004_2024:
    # 使用第一个年份的元数据
    first_year = years_2004_2024[0]
    save_tiff(output_file_2004_2024, max_data_2004_2024, yearly_max[first_year]['meta'])
